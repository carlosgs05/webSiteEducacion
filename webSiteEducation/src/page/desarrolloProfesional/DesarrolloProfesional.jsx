...
const DesarrolloProfesional = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:8000/api/desarrolloProfesional")
      .then((response) => setDataDesarrollo(response.data))
      .catch((error) => console.error("Error al obtener los datos:", error));
  }, []);

  // Filtra los registros según el campo "Tipo"
  const dataPasantias = dataDesarrollo.filter(item => item.Tipo === "pasantias");
  const dataRsu = dataDesarrollo.filter(item => item.Tipo === "rsu");
  const dataBolsaTrabajo = dataDesarrollo.filter(item => item.Tipo === "bolsa de trabajo");
  ...
  return (
    <>
      ...
      <section className={`${activeSection === "pasantias" ? "block" : "hidden"} my-10`}>
        <CardSlider
      ...
      {/* Sección de RSU */}
      <section className={`${activeSection === "rsu" ? "block" : "hidden"} my-10`}>
        <CardSlider
      ...
      <section className={`${activeSection === "bolsaTrabajo" ? "block" : "hidden"} my-10`}>
        <CardSlider
          title="Bolsa de Trabajo"
          data={dataBolsaTrabajo}
        />
      </section>
