  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        "https://pagina-educacion-backend-production.up.railway.app/api/documentos"
      );
  ...
  const handleDelete = (id) => {
    swal({
      ...
      if (willDelete) {
        axios
          .delete(
            `https://pagina-educacion-backend-production.up.railway.app/api/destroyDocumento/${id}`
          )
          .then(() => {
        ...
        {data.length > itemsPerPage && (
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6">
            <div className="text-gray-600 text-sm">
              ...
              {Math.min(indexOfLastItem, data.length)} de {data.length}{" "}
              registros
            </div>
